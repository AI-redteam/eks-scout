...
.header-title {
  font-size: 32px; font-weight: 800; letter-spacing: -0.5px; line-height: 1.1;
  background: linear-gradient(135deg, #58a6ff 0%, #d2a8ff 50%, #f78166 100%);
  -webkit-background-clip: text; -webkit-text-fill-color: transparent;
  background-clip: text;
}
.header-byline {
  font-size: 11px; color: #8b949e; letter-spacing: 3px; text-transform: uppercase;
  font-weight: 500;
}
.header-cluster {
  font-size: 16px; color: #c9d1d9; font-weight: 500; margin-top: 4px;
  padding-left: 2px;
}
...
/* === Attack Chain === */
.attack-chain { margin-bottom: 16px; }
.attack-chain-label {
...
.chain-node {
  flex: 0 0 auto; min-width: 140px; max-width: 180px; border-radius: 8px;
  padding: 12px 14px; position: relative; display: flex; flex-direction: column;
...
.chain-node-desc {
  font-size: 10px; opacity: 0.75; line-height: 1.3;
}
.chain-node.node-entry {
  background: #58a6ff18; border: 1px solid #58a6ff55;
}
.chain-node.node-entry .chain-node-phase,
.chain-node.node-entry .chain-node-title { color: #58a6ff; }
.chain-node.node-entry .chain-node-desc { color: #79c0ff; }
.chain-node.node-vuln {
  background: #f0883e18; border: 1px solid #f0883e55;
}
.chain-node.node-vuln .chain-node-phase,
.chain-node.node-vuln .chain-node-title { color: #f0883e; }
.chain-node.node-vuln .chain-node-desc { color: #ffb86c; }
.chain-node.node-escalate {
  background: #d2992218; border: 1px solid #d2992255;
}
.chain-node.node-escalate .chain-node-phase,
.chain-node.node-escalate .chain-node-title { color: #d29922; }
.chain-node.node-escalate .chain-node-desc { color: #e3b341; }
.chain-node.node-goal {
  background: #f8514918; border: 1px solid #f8514955;
}
.chain-node.node-goal .chain-node-phase,
.chain-node.node-goal .chain-node-title { color: #f85149; }
.chain-node.node-goal .chain-node-desc { color: #ff7b72; }
.chain-arrow {
  flex: 0 0 auto; display: flex; align-items: center; padding: 0 2px;
  color: #484f58; font-size: 18px; user-select: none;
}
.chain-arrow svg { width: 28px; height: 20px; }
.chain-arrow .arrow-line { stroke: #30363d; stroke-width: 2; }
.chain-arrow .arrow-head { fill: #484f58; }
...
@media (max-width: 768px) {
  ...
  .chain-flow { flex-wrap: wrap; gap: 4px; }
...
<div class="container">
  ...
  <div class="header">
    <div class="header-brand">
      <div class="header-title">EKS Scout</div>
      <div class="header-byline">by Ben Stevens</div>
      <div class="header-cluster">{_escape_html(cluster)}</div>
    </div>
  ...
  <div class="footer">
    Generated by EKS Scout by Ben Stevens &bull; <span id="gen-time"></span>
  </div>
</div>
...
(function() {
  ...
  // --- Attack Chain Knowledge Base ---
  // Maps finding types to kill-chain phases, capabilities, and sort order.
  // phase 1 = initial position, 2 = escalation, 3 = breakout/lateral movement
  var ATTACK_KB = {
    "Container Running As Root":              { phase: 1, label: "Root User",          capability: "Unrestricted container operations", nodeClass: "node-entry" },
    "Container Allowed to Run As Root":       { phase: 1, label: "Root Allowed",       capability: "Can gain UID 0 at runtime",        nodeClass: "node-entry" },
    "Container May Run As Root":              { phase: 1, label: "Root Possible",      capability: "No runAsNonRoot enforcement",       nodeClass: "node-entry" },
    "Container Root Filesystem Writable":     { phase: 1, label: "Writable FS",        capability: "Install tools, modify binaries, persist", nodeClass: "node-entry" },
    "Container Allows Privilege Escalation":  { phase: 2, label: "Priv Escalation",    capability: "SUID/kernel exploit to gain root",  nodeClass: "node-escalate" },
    "Dangerous Capabilities Added":           { phase: 2, label: "SYS_ADMIN Cap",      capability: "Namespace manipulation, mount, breakout", nodeClass: "node-escalate" },
    "Privileged Container":                   { phase: 2, label: "Privileged Mode",    capability: "Full kernel access, device control", nodeClass: "node-escalate" },
    "Pod Using HostPath Volume":              { phase: 3, label: "Host Filesystem",    capability: "Read/write node files, steal creds", nodeClass: "node-vuln" },
    "Pod Using Host Network":                 { phase: 3, label: "Host Network",       capability: "Sniff traffic, access node services", nodeClass: "node-vuln" },
    "Pod Using Host PID Namespace":           { phase: 3, label: "Host PID",           capability: "See/signal all node processes",     nodeClass: "node-vuln" },
    "Pod IRSA Role Potentially Overly Permissive": { phase: 3, label: "Overprivileged AWS Role", capability: "Abuse IAM for cloud resource access", nodeClass: "node-vuln" },
    "Pod Using Host IPC Namespace":              { phase: 3, label: "Host IPC",              capability: "Shared memory access to host processes",    nodeClass: "node-vuln" },
    "IMDSv2 Not Enforced":                       { phase: 3, label: "IMDS v1 Exposed",       capability: "HTTP metadata service leaks node IAM credentials", nodeClass: "node-vuln" },
    "Node IAM Role Has Overly Broad Policy":     { phase: 3, label: "Overprivileged Node",   capability: "Broad AWS permissions inherited on escape",  nodeClass: "node-vuln" },
    "EKS Public API Endpoint Open to Internet":  { phase: 1, label: "Public K8s API",        capability: "API server reachable from internet",         nodeClass: "node-entry" },
    "ClusterRoleBinding Grants High Privileges": { phase: 2, label: "Cluster Admin RBAC",    capability: "Stolen creds grant full cluster control",    nodeClass: "node-escalate" },
    "EKS Secrets Encryption Not Enabled":        { phase: 1, label: "Unencrypted etcd",      capability: "Secrets stored plaintext at rest",           nodeClass: "node-entry" },
    "Secret Contains Sensitive-Looking Keys":    { phase: 3, label: "Sensitive Secrets",      capability: "Credentials, tokens, API keys in K8s secrets", nodeClass: "node-vuln" },
    "Nodegroup SSH Access Enabled Without Source Restriction": { phase: 1, label: "Open SSH", capability: "SSH reachable from any IP",                  nodeClass: "node-entry" }
  };

  // Maps combo titles to attacker goals. Falls back to a generic derivation.
  var GOAL_KB = {
    "Privileged Container with Host Filesystem Access":   { goal: "Node Compromise",        detail: "Escape container, full control of host node" },
    "Privileged Container with Host Network Access":      { goal: "Network Takeover",        detail: "Control node network stack, sniff all traffic" },
    "SYS_ADMIN Capability with Host Filesystem Access":   { goal: "Node Compromise",        detail: "Mount manipulation to escape and own the host" },
    "SYS_ADMIN Capability with Host Network Access":      { goal: "Node Compromise",        detail: "Kernel-level network + privilege = full node control" },
    "Root Container with Host Filesystem Access":         { goal: "Credential Theft",       detail: "Read kubelet creds, service account tokens, secrets" },
    "Root Container with Host Network Access":            { goal: "Lateral Movement",       detail: "Access metadata API, sniff pod-to-pod traffic" },
    "Root Container with Host PID Visibility":            { goal: "Process Hijacking",      detail: "Attach to / signal host processes, extract secrets" },
    "Root Container with Overprivileged AWS Role":        { goal: "Cloud Compromise",       detail: "Pivot from container to AWS account resources" },
    "Multiple Host Namespace Breakouts":                  { goal: "Full Node Access",       detail: "Combined breakouts collapse container isolation" },
    "Host Network with Host Filesystem Access":           { goal: "Data Exfiltration",      detail: "Read sensitive files + unrestricted network egress" },
    "Host PID Visibility with Host Filesystem Access":    { goal: "Container Escape",       detail: "Process + filesystem access enables escape" },
    "Host Filesystem Access with Privilege Escalation":   { goal: "Container Escape",       detail: "Escalate then write to host for breakout" },
    "Root Container with Privilege Escalation Allowed":   { goal: "Kernel Exploitation",    detail: "SUID binaries + root enable kernel exploits" },
    "Root Container with Writable Filesystem":            { goal: "Persistent Backdoor",    detail: "Install tools and maintain access across restarts" },
    "Privileged Container with Host IPC Access":           { goal: "Node Compromise",        detail: "Full privileges + shared memory = complete node control" },
    "SYS_ADMIN Capability with Host IPC Access":           { goal: "Node Compromise",        detail: "Kernel caps + IPC = manipulate host process memory" },
    "Root Container with Host IPC Access":                 { goal: "Process Manipulation",   detail: "Attach to host shared memory segments as root" },
    "Host IPC with Host PID Visibility":                   { goal: "Process Hijacking",      detail: "See all processes + manipulate their shared memory" },
    "Host IPC with Host Filesystem Access":                { goal: "Data Theft",             detail: "Read host files + exfil via shared memory channels" },
    "Host IPC with Host Network Access":                   { goal: "Full Host Interaction",  detail: "IPC + network = interact with host on all channels" },
    "IMDS Credential Theft via Host Network":              { goal: "Cloud Credential Theft", detail: "Steal node IAM creds via EC2 metadata service" },
    "IMDS Credential Theft via Privileged Container":      { goal: "Cloud Credential Theft", detail: "Privileged access to network + IMDS = node IAM creds" },
    "Overprivileged Node Role with Container Escape":      { goal: "Cloud Account Compromise", detail: "Escape to node, inherit broad AWS permissions" },
    "Overprivileged Node Role with Host Filesystem Access": { goal: "Cloud Credential Theft", detail: "Read node IAM creds from filesystem, abuse broad role" },
    "Overprivileged Node Role with Host Network Access":   { goal: "Cloud Credential Theft", detail: "Network access to IMDS + broad node role = AWS pivot" },
    "Public API Endpoint with Cluster Admin Privileges":   { goal: "Cluster Takeover",       detail: "Internet-exposed API + cluster-admin creds = full control" },
    "Unencrypted Secrets with Sensitive Data":             { goal: "Credential Exposure",    detail: "Sensitive secrets readable in plaintext from etcd" },
    "Unrestricted SSH with Overprivileged Node Role":      { goal: "Full Node + Cloud Compromise", detail: "SSH brute force \u2192 node \u2192 broad AWS permissions" }
  };

  function buildAttackChain(combo) {
    // Build ordered chain nodes from the combo's matched finding types
    var steps = [];
    (combo.matched_finding_types || []).forEach(function(ft) {
      var kb = ATTACK_KB[ft];
      if (kb) {
        steps.push({ phase: kb.phase, label: kb.label, capability: kb.capability, nodeClass: kb.nodeClass, type: ft });
      } else {
        // Unknown finding type — still show it
        steps.push({ phase: 2, label: ft.replace(/^(Container |Pod )/, ''), capability: 'Security misconfiguration', nodeClass: 'node-vuln', type: ft });
      }
    });
    // Sort by phase so the chain flows logically
    steps.sort(function(a, b) { return a.phase - b.phase; });

    // Determine attacker goal
    var goalInfo = GOAL_KB[combo.title];
    if (!goalInfo) {
      // Derive from impact text
      goalInfo = { goal: 'Compromise', detail: (combo.impact || '').substring(0, 80) };
    }

    return { steps: steps, goal: goalInfo };
  }

  var PHASE_LABELS = { 1: 'Foothold', 2: 'Escalation', 3: 'Breakout' };

  function renderChainArrow() {
    var arrow = document.createElement('div');
    arrow.className = 'chain-arrow';
    arrow.innerHTML = '<svg viewBox="0 0 28 20"><line class="arrow-line" x1="0" y1="10" x2="20" y2="10"/><polygon class="arrow-head" points="18,5 28,10 18,15"/></svg>';
    return arrow;
  }

  function renderAttackChain(combo) {
    var chain = buildAttackChain(combo);
    var container = document.createElement('div');
    container.className = 'attack-chain';

    var label = document.createElement('div');
    label.className = 'attack-chain-label';
    setText(label, 'Attack Path');
    container.appendChild(label);

    var flow = document.createElement('div');
    flow.className = 'chain-flow';

    // Entry point node (always present — represents initial code execution)
    var entry = document.createElement('div');
    entry.className = 'chain-node node-entry';
    entry.innerHTML = '<div class="chain-node-phase">Initial Access</div>';
    var entryTitle = document.createElement('div');
    entryTitle.className = 'chain-node-title';
    setText(entryTitle, 'Code Execution');
    entry.appendChild(entryTitle);
    var entryDesc = document.createElement('div');
    entryDesc.className = 'chain-node-desc';
    setText(entryDesc, 'Attacker runs code in container');
    entry.appendChild(entryDesc);
    flow.appendChild(entry);

    // Vulnerability step nodes
    chain.steps.forEach(function(step) {
      flow.appendChild(renderChainArrow());
      var node = document.createElement('div');
      node.className = 'chain-node ' + step.nodeClass;
      var phaseEl = document.createElement('div');
      phaseEl.className = 'chain-node-phase';
      setText(phaseEl, PHASE_LABELS[step.phase] || 'Exploit');
      node.appendChild(phaseEl);
      var titleEl = document.createElement('div');
      titleEl.className = 'chain-node-title';
      setText(titleEl, step.label);
      node.appendChild(titleEl);
      var descEl = document.createElement('div');
      descEl.className = 'chain-node-desc';
      setText(descEl, step.capability);
    ...
    goalNode.className = 'chain-node node-goal';
    goalNode.innerHTML = '<div class="chain-node-phase">Attacker Goal</div>';
    var goalTitle = document.createElement('div');
    goalTitle.className = 'chain-node-title';
    setText(goalTitle, chain.goal.goal);
    goalNode.appendChild(goalTitle);
    var goalDesc = document.createElement('div');
    goalDesc.className = 'chain-node-desc';
    setText(goalDesc, chain.goal.detail);
    goalNode.appendChild(goalDesc);
    flow.appendChild(goalNode);
  ...
  function renderCombos() {
    ...
    combos.forEach(function(combo) {
      ...
      // expandable detail with attack chain + contributing findings
      var detail = document.createElement('div');
      detail.className = 'combo-detail';

      // Attack chain visualization
      detail.appendChild(renderAttackChain(combo));
